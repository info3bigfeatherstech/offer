...
const VariantModal = ({
  ...
  const isWholesaleEligible = () => {
    return variantForm.wholesale && variantForm.wholesaleBase && parseFloat(variantForm.wholesaleBase) > 0;
  };
  ...
  const handleSave = () => {
    ...
    // Determine wholesale visibility based on eligibility
    const wholesaleEligible = isWholesaleEligible();
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full my-8 shadow-2xl">
        <div className="p-5 border-b border-gray-200 flex items-center justify-between bg-indigo-50 rounded-t-2xl">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{isEditing ? 'Update price, inventory & attributes — ProductCode is locked after creation' : 'Set ProductCode, attributes, price & stock'}</p>
          </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div>
            <label className="text-sm font-semibold text-gray-800 mb-2 block">
              ...
              {isEditing ? <span className="ml-2 text-xs font-normal text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">🔒 Locked — identifies variant in DB</span> : <span className="ml-2 text-xs font-normal text-gray-400">Unique number — required</span>}
            </label>
            <div className="relative">
              <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5h2M3 9h2M3 13h2M3 17h2M3 19h2 M7 5v14M10 5v14M13 5v4M13 11v8 M16 5v14M19 5h2M19 9h2M19 13h2M19 17h2M19 19h2" /></svg>
              </div>
              <input type="text" inputMode="numeric" value={variantForm.ProductCode ?? ''} onChange={(e) => setVariantForm(prev => ({ ...prev, ProductCode: e.target.value }))} disabled={isEditing} className={`w-full pl-10 pr-10 py-2.5 border rounded-lg text-sm font-mono tracking-widest transition-colors ${isEditing ? 'bg-gray-100 border-gray-300 text-gray-500 cursor-not-allowed' : !(variantForm.ProductCode ?? '').toString().trim() ? 'bg-red-50 border-red-300 focus:ring-2 focus:ring-red-400 focus:bg-white' : 'bg-gray-50 border-gray-200 focus:ring-2 focus:ring-indigo-400 focus:bg-white'}`} placeholder="e.g., 1234567890128" maxLength={20} />
              {!isEditing && (variantForm.ProductCode ?? '').toString().trim() && (
                <button type="button" onClick={() => setVariantForm(prev => ({ ...prev, ProductCode: '' }))} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-red-500">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              )}
            </div>
            {(variantForm.ProductCode ?? '').toString().trim() ? (
              <p className={`text-xs mt-1.5 font-mono ${isEditing ? 'text-gray-500' : 'text-indigo-600'}`}>📦 {variantForm.ProductCode}{isEditing && <span className="ml-2 font-sans text-gray-400">(locked)</span>}</p>
            ) : !isEditing ? <p className="text-xs text-red-500 mt-1.5">⚠️ Backend will reject without a Productcode</p> : null}
          </div>

          {/* Attributes */}
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="text-sm font-semibold text-gray-800">Variant Attributes <span className="text-gray-400 font-normal text-xs ml-1">(optional)</span></label>
              <button type="button" onClick={addVariantAttribute} className="text-xs px-3 py-1 bg-indigo-100 text-indigo-700 rounded-lg hover:bg-indigo-200 font-medium">+ Add</button>
            </div>
            <div className="space-y-2">
              {variantForm.attributes.map((attr, index) => (
                <div key={index} className="flex gap-2 items-center">
                  <input type="text" value={attr.key} onChange={(e) => updateVariantAttribute(index, 'key', e.target.value)} className="flex-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-400 focus:bg-white" placeholder="Key (e.g., Color)" />
                  <input type="text" value={attr.value} onChange={(e) => updateVariantAttribute(index, 'value', e.target.value)} className="flex-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-400 focus:bg-white" placeholder="Value (e.g., Blue)" />
                  {variantForm.attributes.length > 1 && (
                    <button type="button" onClick={() => removeVariantAttribute(index)} className="p-2 text-gray-400 hover:text-red-500 rounded-lg hover:bg-red-50">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                  )}
                </div>
          ...
          {/* Ecom Visibility Toggle - NEW */}
          <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
            <div>
              <label className="text-sm font-semibold text-gray-800">Ecom Visibility</label>
              <p className="text-xs text-gray-500 mt-0.5">Show this variant on the ecommerce storefront</p>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-500">{variantForm.channelVisibility?.ecomm === "active" ? "Active" : "Draft"}</span>
              <button
                type="button"
          ...
          {/* Pricing */}
          <div>
            <label className="text-sm font-semibold text-gray-800 mb-3 block">Variant Pricing (₹) <span className="text-red-400">*</span></label>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Base Price (₹)</label>
                <input type="number" value={variantForm.price.base} onChange={(e) => setVariantForm(prev => ({ ...prev, price: { ...prev.price, base: e.target.value } }))} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-400 focus:bg-white" placeholder="89000" />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Sale Price (₹) <span className="text-gray-400">(optional)</span></label>
                <input type="number" value={variantForm.price.sale ?? ''} onChange={(e) => setVariantForm(prev => ({ ...prev, price: { ...prev.price, sale: e.target.value } }))} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-400 focus:bg-white" placeholder="79000" />
              </div>
            </div>
          ...
          <div className="border-t border-gray-200 pt-4">
            ...
            {/* Wholesale Visibility Badge (read-only, auto-calculated) */}
            <div className={`flex items-center justify-between p-3 rounded-lg mb-3 ${isWholesaleEligible() ? "bg-purple-50 border border-purple-200" : "bg-gray-50 border border-gray-200"}`}>
              <div>
                <label className="text-xs font-medium text-gray-700">Wholesale Visibility</label>
                <p className="text-xs text-gray-500 mt-0.5">
                  {isWholesaleEligible() ? "Auto-activated (wholesale=true + wholesaleBase>0)" : "Requires wholesale=true and wholesaleBase>0"}
                </p>
              </div>
              <div>
              ...
              <div className="space-y-3 p-3 bg-purple-50 rounded-lg border border-purple-200">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" value={variantForm.wholesaleBase || ''} onChange={(e) => setVariantForm(prev => ({ ...prev, wholesaleBase: e.target.value }))} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-purple-400" placeholder="e.g., 75000" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">Wholesale Sale Price (₹)</label>
                    <input type="number" value={variantForm.wholesaleSale || ''} onChange={(e) => setVariantForm(prev => ({ ...prev, wholesaleSale: e.target.value }))} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-purple-400" placeholder="e.g., 72000" />
                  </div>
                </div>
                <div>
                  ...
                  <input type="number" min="1" value={variantForm.minimumOrderQuantity || 1} onChange={(e) => setVariantForm(prev => ({ ...prev, minimumOrderQuantity: parseInt(e.target.value) || 1 }))} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-purple-400" placeholder="Minimum quantity for wholesale price" />
                </div>
                {variantForm.wholesaleBase && variantForm.wholesaleSale && Number(variantForm.wholesaleSale) < Number(variantForm.wholesaleBase) && (
            ...
            {variantForm.inventory.trackInventory && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={variantForm.inventory.quantity} onChange={(e) => setVariantForm(prev => ({ ...prev, inventory: { ...prev.inventory, quantity: parseInt(e.target.value) || 0 } }))} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-400 focus:bg-white" placeholder="25" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">Low Stock Threshold</label>
                  <input type="number" value={variantForm.inventory.lowStockThreshold} onChange={(e) => setVariantForm(prev => ({ ...prev, inventory: { ...prev.inventory, lowStockThreshold: parseInt(e.target.value) || 5 } }))} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-400 focus:bg-white" placeholder="5" />
                </div>
              </div>
            )}
          </div>
          ...
          {/* Images */}
          <div>
            <label className="text-sm font-semibold text-gray-800 mb-3 block">Variant Images <span className="text-gray-400 text-xs font-normal">(up to 4)</span></label>
            <label className={`block w-full border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition-all mb-3 ${variantImageDragging ? 'border-indigo-500 bg-indigo-50' : 'border-gray-300 hover:border-indigo-400'}`} onDragOver={(e) => { e.preventDefault(); setVariantImageDragging(true); }} onDragLeave={() => setVariantImageDragging(false)} onDrop={(e) => { e.preventDefault(); setVariantImageDragging(false); handleVariantImageUpload({ target: { files: e.dataTransfer.files } }); }}>
              ...
              <svg className="w-7 h-7 mx-auto mb-1 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
            ...
            {variantForm.images.length > 0 && (
              <div className="grid grid-cols-4 gap-2">
                {variantForm.images.map((image) => {
                  ...
                  return (
                    <div key={imgId} className={`relative rounded-lg overflow-hidden border-2 ${image.isMain ? 'border-indigo-500' : 'border-gray-200'}`}>
                      ...
                      <div className="absolute inset-0 bg-black bg-opacity-0 hover:bg-opacity-30 transition-all flex items-center justify-center gap-1 opacity-0 hover:opacity-100">
                        {!image.isMain && (
                          <button type="button" onClick={() => setVariantMainImage(imgId)} className="p-1 bg-white rounded-full text-indigo-600">
                        ...
                        <button type="button" onClick={() => removeVariantImage(imgId)} className="p-1 bg-white rounded-full text-red-500">
        ...
        <div className="p-5 border-t border-gray-100 flex flex-col gap-3">
          {saveError && (<div className="p-3 bg-red-50 border border-red-200 rounded-lg"><p className="text-red-700 text-sm font-medium">❌ {saveError}</p></div>)}
          <div className="flex justify-end gap-3">
            <button type="button" onClick={onClose} disabled={isSaving} className="px-5 py-2.5 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 disabled:opacity-60 transition-colors">Cancel</button>
            <button type="button" onClick={handleSave} disabled={isSaving} className="px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold rounded-lg hover:from-indigo-600 hover:to-purple-700 disabled:opacity-60 flex items-center gap-2 transition-all">
              {isSaving ? <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />{isEditing ? "Updating..." : "Saving..."}</> : isEditing ? "✓ Update Variant" : "✓ Save Variant"}
            </button>
          </div>
        </div>
      </div>
...
// code is working but upper code have wholeseller sepration 
// // Shared_components/VariantModal.jsx
// //
// // Used for:
